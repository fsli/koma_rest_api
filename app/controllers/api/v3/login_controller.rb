class Api::V3::LoginController < ApplicationController
  ...
  before_filter  :authenticate
      ...
      def create
        ...
        if ret[:result]
          ...
          user = KomaUser.find_by(username: param_username)
          result = true
          if user
            salt = user['salt']
          ...
          if result
             render json: {result: result, user_id: session[:user_id], role: session[:role_id], message: "User is already logged in."}
          else
