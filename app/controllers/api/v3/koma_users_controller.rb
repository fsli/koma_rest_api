class Api::V3::KomaUsersController < ApplicationController
    skip_before_filter  :verify_authenticity_token
    before_filter :authenticate
    def index
      user_id = session[:user_id]
      data = KomaUser.all.select(:id,:username, :attr, :created_at, :updated_at, :company_id, :nickname, :picture_url, :facebook_id, :badge_number).where("id = ?", user_id).order("created_at DESC")
      results = Array.new()
      data.each do |row|
