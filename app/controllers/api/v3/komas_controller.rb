class Api::V3::KomasController < ApplicationController
  ...
  before_filter :authenticate
      ...
      def create
        param_owner_id = session[:user_id]
      ...
      def update
        ...
        if ret[:result]
          koma = Koma.find("id = ? AND owner_id = ? ", param_koma_id, param_owner_id)
          if koma == nil
            render json: {result: false, message: "Koma id: #{param_koma_id} cannot be found."}   
            return
          end
          ret = update_koma(koma, param_koma_date, param_koma_type, param_prospect_name, param_memo) 
      ...
      private def update_koma(data, koma_date, koma_type, prospect_name, memo) 
        data.update(koma_date: DateTime.strptime(koma_date, "%m/%d/%Y %H:%M"), koma_type: koma_type, prospect_name: prospect_name, memo: memo)
