class Api::V3::KomaMessagesController < ApplicationController
  ...
  def index
    ...
    if !user_id.nil? 
      conditions += " user_id = :user_id OR sender_user_id = :user_id "
  ...
  def show
    ...
    joined_tables = 'LEFT OUTER JOIN koma_users ON koma_messages.user_id = koma_users.id LEFT OUTER JOIN koma_users B on koma_messages.sender_user_id = B.id'
    row = KomaMessage.joins(joined_tables).select(selected_columns).find("where user_id = ? OR sender_user_id = ?", user_id, user_id)
    if row == nil
  ...
  def create
    ...
    param_sender_user_id = session[:user_id]
  ...
  def destroy
    ...
    user_id = session[:user_id]
    data = KomaMessage.find("id = ? AND (sender_user_id = ? OR user_id = ?)", user_id, user_id)
    if data == nil
      render json: {result: false, message: "Koma Message id: #{param_id} cannot be found."}    
      return
    end 
